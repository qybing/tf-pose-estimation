...
def dup_person(persons_num):
    ...
    for i in list1:
        d.append(len(i))
    dic = list1[d.index(max(d))]
    del list1[d.index(max(d))]
    for i in list1:
        e = 0
        f = len(i)
        for (key, value) in i.items():
            if key in dic.keys() and dic[key] == value:
                e = e + 1
            if e == f:
                start = start - 1
    return start
...
def gen_rect(centers):
    """
    get rectangle centers
    :param centers:
    :return: judge person is or isn't fall
    """
    x_aount = []
    y_aount = []
    cnt_center = []
    for (key, values) in centers.items():
        a = (values[0], values[1])
        cnt_center.append(a)
        x_aount.append(values[0])
        y_aount.append(values[1])
    x_max = max(x_aount)
    y_max = max(y_aount)
    x_min = min(x_aount)
    y_min = min(y_aount)
    h = y_max - y_min
    w = x_max - x_min
    nu = x_max - (x_max + x_min) / 2
    de = y_max - (y_max + y_min) / 2
    b = math.atan(abs(nu / de))
    hor = b / math.pi * 180
    if w == 0:
        print('w is except')
        return False
    print("h:w = {}，angle：{}".format(float(h) / w, hor))
    if float(h) / w < 1.10 or hor > 45:
        return True
    else:
        return False


def draw_feature(persons_sum, npimg):
    list1 = persons_sum
    start = len(list1)
    if start <= 0:
        return 0
...
class TfPoseEstimator:
    ...
    def __init__(self, graph_path, target_size=(320, 240), tf_config=None):
        ...
        # self.persistent_sess = tf.Session(target="grpc://10.10.1.7:2222", graph=self.graph, config=tf_config)
    ...
    @staticmethod
    def draw_humans(npimg, humans, imgcopy=False, video_name=None):
        if imgcopy:
            npimg = np.copy(npimg)
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                centers[i] = center
            qiao = copy.copy(centers)
            persons_sum.append(qiao)
            for pair_order, pair in enumerate(common.CocoPairsRender):
                if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                    continue
        is_fall = draw_feature(persons_sum, npimg)
        persons_num = dup_person(persons_sum)
        return npimg, persons_num, is_fall
